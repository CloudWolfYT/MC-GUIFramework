item modify block ~ ~ ~ container.11 a:remove_one
...
execute store result score #temp ui run data get storage ui container[{Slot:11b}].Count
execute if score #temp ui matches 1 run data remove storage ui load